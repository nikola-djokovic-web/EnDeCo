<?php
    return [
        'home' => "Početna",
        'about-us' => "About Us",
        'projects' => "Projekti",
        'news' => "News",
        'contact' => "Kontaktirajte nas",
        'eng-solutions' => 'Inženjerska rešenja',
        'plant' => 'Dizajn postrojenja',
        'consulting' => 'Konsultantske usluge',
        'who-we-are' => 'Ko smo mi',
        'technical-revisions' => 'Tehničke revizije',
        'industries' => 'Industrije',
        'where' => 'Gde se nalazimo',
        'who-we-are-description' => 'Da li planirate da izgradite, optimizujete ili proširite postrojenje ili cevovodni sistem?
            EnDeCo nudi izuzetne profesionalne inženjerske, dizajnerske i prostorno planske usluge otvorene širokom spektru industrija. <br> U EnDeCo, možete računati na iskusan i visoko efikasan tim stručnjaka koji su posvećeni i kvalifikovani da dizajniraju najbolje postrojenje za vaše individualne potrebe. <br> <br> Posvećeni smo izvrsnosti i integritetu, istovremeno    nastojeći da izgradimo dugoročne poslovne odnose sa našim klijentima. U EnDeCo, sve je orijentisano ka kupcu.
            Razgovarajmo o najboljem rešenju za vaše potrebe - rado ćemo vam pomoći!',
        'plant-description' => 'Dizajn postrojenja je jedan od naših glavnih poslova - detaljna i individualna rešenja iz jednog izvora.
            Već dugi niz godina, EnDeCoekperts su bili vredan partner u konsaltingu i dizajnu postrojenja za kompanije iz različitih sektora.
            <br> Naše usluge uključuju: <br> <br> • Inženjering, <br> • P & ID <br> • Dizajn industrijskih postrojenja <br> • Priprema tehničke dokumentacije <br> • Piping, itd.',
        'consulting-description' => 'U smislu našeg pristupa konsaltingu, EnDeCo tretira svaki slučaj kao da smo morali sami da gradimo fabriku
            na osnovu našeg saveta - bez obzira da li će treća strana voditi računa o izgradnji postrojenja.
            Bez obzira da li vam je potreban ekspertski izveštaj, studija izvodljivosti ili definicija trenutnog i ciljnog stanja, u EnDeCo-u, koristićemo svo naše iskustvo i znanje da bismo došli do cilja.',
        'technical-revisions-description' => 'EnDeCo eksperti imaju veliko iskustvo u tehničkim kontrolama dizajnerskih projekata i glavnih projekata u različitim industrijskim oblastima.',
        'helpline' => 'Naša linija za pomoć je uvek otvorena za primanje bilo kakvih upita ili povratnih informacija.',
        'yourname' => 'Vaše ime',
        'youremail' => 'Vaš Email',
        'contactphone' => 'Vaš kontakt telefon',
        'comment' => 'Napišite komentar ovde ...',
        'submit' => 'Pošaljite',
        'address' => 'Adresa',
        'contacts' => 'Kontakt',
        'socials' => 'Socijalne mreže',
        'process-eng' => 'Procesno <br>inženjerstvo',
        'chemical-eng' => 'Hemijsko <br>inženjerstvo',
        'water' => 'Prečišćavanje <br>vode',
        'food' => 'Proizvodnja hrane',
        'oil' => 'Nafta i gas',
        'minerals' => 'Minerali',
        'cement' => 'Proizvodnja <br>cementa',
        'life' => 'Life sciences',
        'contact-message' => 'Poruka uspešno poslata.',


    ];
?>
